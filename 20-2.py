...
for i in nodes.values():
    if isinstance(i,Conjunction):
        i.get_inputs()
first_1_output={name:0 for name in nodes['df'].inputs}
step=0
while any([first_1_output[x]==0 for x in first_1_output]):
    step+=1
    queue=[('br',i,0) for i in broadcaster]
    for current in queue:
        try:
            nodes[current[1]].recieve(current)
            if current[0] in first_1_output.keys() and current[2]==1:
                first_1_output[current[0]]=step
        except KeyError:
            continue
    for name,val in first_1_output.items():
        if nodes[name].last_output==1:
            print('here!')
        first_1_output[name]=step if nodes[name].last_output==1 and val==0 else val

count_all[0]+=(len(broadcaster)+1)*1000
print(count_all[0]*count_all[1],first_1_output)
print(lcm(*[i for i in first_1_output.values()]))